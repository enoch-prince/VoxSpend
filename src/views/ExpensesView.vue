  ...
  <div class="expenses-view overflow-y-auto h-full">
    <div class="expenses-view__content px-lg py-md">
      <header class="flex justify-between items-center mb-lg">
        ...
        <div class="expenses-view__month-select neo-chip" @click="showMonthPicker = !showMonthPicker">
      ...
      <div class="flex flex-col gap-sm">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <p class="text-xs text-tertiary">{{ cat.count }} transaction{{ cat.count !== 1 ? 's' : '' }}</p>
          </div>
...
const monthExpenses = computed(() =>
  expensesStore.getExpensesForMonth(selectedYear.value, selectedMonth.value)
...
const breakdown = computed((): CategoryBreakdown[] => {
  ...
  return Object.entries(grouped)
    .map(([category, data]) => {
      ...
      return {
        category, color: cat?.color || '#64748B', icon: cat?.icon || 'more_horiz',
        total: data.total, percentage: total > 0 ? Math.round((data.total / total) * 100) : 0, count: data.count
      }
    })
