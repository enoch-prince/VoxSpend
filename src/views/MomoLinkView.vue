        ...
        >
          <div class="flex items-center gap-md">
            <div class="momo-view__logo" :style="{ background: provider.color + '20' }">
              <span class="font-bold text-sm" :style="{ color: provider.color }">{{ provider.abbr }}</span>
            </div>
            <div class="flex-1">
          ...
          <div v-for="acc in getAccountsFor(provider.key)" :key="acc.id" class="momo-view__linked mt-md">
            <div class="flex items-center gap-sm">
              ...
              <button class="neo-button neo-button--ghost" style="margin-left:auto;padding:4px;" @click="unlinkAccount(acc.id)">
          ...
          <div v-if="linkingProvider === provider.key" class="momo-view__form mt-md">
            <input v-model="linkPhone" class="neo-input mb-sm" placeholder="Phone number (e.g. 024XXXXXXX)" type="tel" />
            <input v-model="linkNickname" class="neo-input mb-sm" placeholder="Nickname (optional)" />
            <div class="flex gap-sm">
              <button class="neo-button neo-button--ghost flex-1" @click="linkingProvider = null">Cancel</button>
              <button class="neo-button neo-button--primary flex-1" @click="linkAccount(provider.key)" :disabled="!linkPhone.trim()">Link</button>
            </div>
          </div>
...
const providers = [
  { key: 'mtn' as MomoProvider, name: 'MTN Mobile Money', abbr: 'MTN', color: '#FFCC00', desc: 'MoMo by MTN Ghana' },
  { key: 'telecel' as MomoProvider, name: 'Telecel Cash', abbr: 'TEL', color: '#E40521', desc: 'Cash by Telecel Ghana' },
  { key: 'airteltigo' as MomoProvider, name: 'AirtelTigo Money', abbr: 'AT', color: '#E40000', desc: 'Money by AirtelTigo' }
]
...
<style lang="scss">
.momo-view {
  &__provider { border-radius: $radius-lg; }
  &__logo {
